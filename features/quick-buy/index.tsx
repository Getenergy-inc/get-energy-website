...
export default function QuickBuy() {
  ...
  const buttonsOutput = buttons.map((item) => {
    ...
    return (
      <button className={button_classnames} type="button" key={item._id} onClick={() => goto(item.screen)}>
        {item.title}
      </button>
    );
  });
  ...
  return (
    <div className="w-full">
      <TitleHeader location="Quick Buy" type="2" title="Quick Buy" />
      <div className="w-full flex justify-center my-20">
        <div className="container flex flex-col items-center px-4 md:px-0">
          ...
          <div className="w-full md:w-3/5 my-10 p-6 px-10 text-white main-bg rounded-xl">
            <p>
              Register on the GetEnergy Platform and Download the{" "}
              <Link className="underline" href="/">
                app
              </Link>{" "}
              to enjoy more discounted features, in-app features, pay on credit, etc.
            </p>
          </div>

          <div className="w-4/5 lg:w-3/5 mb-5">{currentScreen}</div>
