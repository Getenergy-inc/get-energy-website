...
export default function HowItWorks() {
  ...
  const renderStepViewer = () => {
    switch (selectedStep) {
      case 1:
        return (
          <TransitionParentInView className="md:mt-16 mt-10 grid md:grid-cols-2 gap-10 md:px-16 px-4">
            ...
            <div>
              <TransitionParentInView className="mt-16 grid md:grid-cols-2 gap-10 px-16">
                <TransitionOpacity className="space-y-10">
                  ...
                  <p className="text-lg leading-relaxed max-w-[30rem]">
                    Experience the power of convenience with GetEnergy&apos;s streamlined Buy, Trade, Recharge services
                    for a hassle-free and efficient experience in purchasing electricity tokens, trading energy assets,
                    and recharging utilities like airtime, data, or cable TV.
                  </p>
                  <button className="px-6 py-3 rounded-full border-2 border-white/50 hover:bg-white hover:text-primaryBlue transition-colors duration-300">
                    Get Started
                  </button>
        ...
        <TransitionParentInViewFast className="grid grid-cols-3 lg:gap-12 md:gap-10 gap-5 mt-10">
          {stepsData.map((data) => (
            <TransitionOpacity key={data.id}>
              ...
              >
                0{data.id}. <span className="hidden md:block">{data.label}</span>
              </button>
            </TransitionOpacity>
          ))}
        </TransitionParentInViewFast>
